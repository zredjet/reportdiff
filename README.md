...
処理中は `処理中 3 / 20 ページ` のように、現在処理しているページの順番と対象数を表示する。`--pages` 指定時は元ページ番号も添える。通常の端末では同じ行を更新し、ログ保存・パイプ・狭い端末では改行して残す。一括比較ではファイル名も表示する。ページ処理後は「レポート出力中」、最後に日本語の結果要約を 1 行表示する。`--quiet` で進捗・要約を抑止できる（エラーは残る）。終了コード `1` は比較が完了して相違が見つかったことを表す。
...
```powershell
.\reportdiff.exe compare old.pdf new.pdf --out result --raw-overlay
```

「確認用オーバーレイ（補正前）」は A のみを赤、B のみを青、共通部分を薄いグレー（既定 `#CCCCCC`）で表示する。位置補正・除外・差分判定を反映せず、元画像を左上で重ねるため、判定で吸収・除外された違いも確認できる。元カラー A/B にも切り替えられる。濃淡への変換で同じグレー値になる色相差は表示できないので、色は元画像で確認する。

同じ自作サンプルの確認用表示。削除した `DRAFT COPY` は赤、追加した `CHECKED` は青、共通の文字や罫線は薄いグレーになる。差分の輪郭・番号・除外色は加えない。

![確認用オーバーレイ。削除したDRAFT COPYは赤、追加したCHECKEDは青、共通部分は薄いグレー](docs/images/sample-raw-overlay.png)
...
共通色は次の YAML で変更できる。`#` をコメントとして扱わせないため、引用符で囲む。`"#000000"` を指定すると旧版の黒／グレー表示を再現できる。色の変更は比較判定に影響しない。
...
- [v0.1.4 の配布準備](docs/verification/v0.1.4-release.md)：ページ進捗・共通色の設定・サンプル・検証・配布範囲
