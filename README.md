...
現在は Phase 0 と T1-1〜T1-5（設定・比較コア・最適化・画像読み込み）が完了。比較コアは 37 件の参照結果と一致し、合成 A4 ページの比較は目標の 2 秒以内（[検証・計測結果](docs/verification/t1-4-core.md)）。PNG・JPEG・BMP・TIFF の読み込みと白背景への透過合成も検証済み（[画像入力の確認結果](docs/verification/t1-5-images.md)）。次は T1-6 の PDF 読み込み。比較コマンドはまだ未実装。
...
GitHub Actions で確認済みの結果は [T0-3](docs/verification/t0-3-ci.md)。T1-1〜T1-5 はローカルの macOS で検証済みで、追加したコードの Windows CI は未実行。
