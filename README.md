...
現在は Phase 0 と T1-1〜T1-7（設定・比較コア・最適化・入力・正規化とページ対応）が完了。比較コアは 37 件の参照結果と一致し、合成 A4 ページの比較は目標の 2 秒以内（[検証・計測結果](docs/verification/t1-4-core.md)）。[画像入力](docs/verification/t1-5-images.md)、[PDF 入力](docs/verification/t1-6-pdf.md)、[サイズ差の白埋め・ページ対応・ページ選択](docs/verification/t1-7-pages.md)も検証済み。次は T1-8 の結果出力。比較コマンドはまだ未実装。
...
GitHub Actions で確認済みの結果は [T0-3](docs/verification/t0-3-ci.md)。T1-1〜T1-7 はローカルの macOS で検証済みで、追加したコードの Windows CI は未実行。
