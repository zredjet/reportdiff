...
確認済み（2026-09-21）：ビルド警告 0・全 450 件成功。37 ゴールデンと最適化前後、分類あり／なしの検出マスク・統計が一致。PDF 結合は既存 6 件＋分類 2 件。macOS Chrome の 1280/390px 表示・操作を確認。A4・300dpi の分類追加処理は約 0.8〜1.0ms、保持する削除マスクは約 8.3MiB。詳細は [T2-1 確認結果](verification/t2-1-classification.md)。T2-2 以降は未着手。
