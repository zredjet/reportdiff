...
T0-3 の GitHub Actions では Windows x64 の疎通テスト 2 件が成功している。T2-5 までを含む版は macOS 上で publish・バンドル・ZIP の静的検証まで完了し、Windows では未実行。以下はユーザーの実機での個別条件として、CI と分けて確認状況を管理する。

- [ ] T2-5 の配布 ZIP を展開し、.NET を入れていない PC で exe が単体で動く
- [ ] 単一 exe から pdfium / OpenCvSharpExtern / libSkiaSharp が展開・ロードされる（T2-5 もバンドル内の x64 と SHA-256 の静的確認まで）
...
確認済み（2026-09-21）：ビルド警告 0・全 700 件成功、追加 60 件（フォント検査 46・実 CLI 14）。既存 37 ゴールデンを維持。自作 TrueType / Type3 の埋め込み・文字取得・描画、元辞書の参照と検査未完了の継続を確認。未使用 Form リソースの循環を修正し回帰検証。macOS Chrome の 1280/390px・長い名前・HTML 特殊文字・キーボード・外部通信なしを確認。A4・600 単語の検査はオープン込み中央値 7.329ms、注釈後の追加分は 0.212ms。Windows exe 156,490,309 bytes、既存依存・ネイティブ DLL のハッシュ・FFmpeg 不在・ZIP の整合を確認。詳細は [T2-5 確認結果](verification/t2-5-fonts.md)。Windows 実行は未確認。T2-6 の前で停止する。
