...
確認済み（2026-09-21）：実効設定・吸収・ノイズ・サイズ差・警告を含む JSON と、ページ画像・重ね描き・切り出しを実装。JSON の項目名と逆シリアライズ、同一ページの保存切り替え、相対パス、除外領域・番号・入れ子の輪郭、切り出しの画素と余白を検証。出力 15 件を追加し、ビルド警告 0・全 340 件成功。D11 の出力を目視でも確認。詳細と出力例は [T1-8 確認結果](verification/t1-8-output.md)。T1-9 の HTML と T1-10 の CLI 接続は未着手。
...
- [ ] T1-1〜T1-8 の追加テストを Windows でも確認する（macOS では 340 件成功）
- [ ] 日本語・空白を含むパス、長いパスで動く（T1-5 の画像・T1-6 の PDF 入力、T1-8 の JSON・PNG 出力の日本語・空白パスは macOS で確認済み）
