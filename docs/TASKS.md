...
確認済み（2026-09-21）：仕様の CLI オプションを入力・比較・全出力に接続。実プロセスの終了コード 0/1/2 と UTF-8、日本語エラー、設定優先順位、ページ選択、force での置き換えと失敗時の旧結果保持を検証。追加 58 件、ビルド警告 0・全 417 件成功。D11 を CLI から比較して相違 2 箇所・終了コード 1、生成 HTML の Chrome 表示も確認。PDF と画像の DPI 不一致をエラーとする条件はユーザー確認のうえ SPEC に追記。詳細は [T1-10 確認結果](verification/t1-10-cli.md)。T1-11 の指定結合シナリオ・T1-12 の配布確認には未着手。
...
- [ ] T1-1〜T1-10 の追加テストを Windows でも確認する（macOS では 417 件成功。リンク 5 件・macOS 固有の Unicode 正規化 1 件は Windows ではスキップし、リンクは下記で実機確認）
- [ ] 日本語・空白を含むパス、長いパスで動く（画像・PDF 入力、JSON・PNG・HTML 出力、T1-10 の CLI 実プロセスを含む日本語・空白パスは macOS で確認済み）
