...
確認済み（2026-09-21）：ビルド警告 0・全 504 件成功。既存 37 ゴールデンと最適化前後、移動注釈あり／なしの検出マスク・削除マスク・統計を維持。PDF 結合は既存 8 件＋移動 4 件。macOS Chrome の 1280/390px と関連リンク・キーボード操作を確認。合成 A4 の 120 図形・240 クラスタの移動は約 476ms、通常の 1 か所変更に対する移動判定の追加処理は約 0.9〜1.4ms。詳細は [T2-2 確認結果](verification/t2-2-movement.md)。T2-3 以降は未着手。
