...
確認済み（2026-09-21）：.NET のストリーム経由で指定ページを白背景の BGR 8bit に描画。3 ページ × 4 DPI の寸法誤差 ±1px、16000px の上限と境界、日本語・空白パス、注釈・フォーム・アンチエイリアスを検証。PDF 入力 36 件を追加し、ビルド警告 0・全 270 件成功。詳細は [T1-6 確認結果](verification/t1-6-pdf.md)。T1-7 以降は未着手。
...
- [ ] T1-1〜T1-6 の追加テストを Windows でも確認する（macOS では 270 件成功）
