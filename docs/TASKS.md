...
T0-3 の GitHub Actions では Windows x64 の疎通テスト 2 件が成功している。T2-4 までを含む版は macOS 上で publish・バンドル・ZIP の静的検証まで完了し、Windows では未実行。以下はユーザーの実機での個別条件として、CI と分けて確認状況を管理する。

- [ ] T2-4 の配布 ZIP を展開し、.NET を入れていない PC で exe が単体で動く
- [ ] 単一 exe から pdfium / OpenCvSharpExtern / libSkiaSharp が展開・ロードされる（T2-4 もバンドル内の x64 と SHA-256 の静的確認まで）
...
確認済み（2026-09-21）：ビルド警告 0・全 589 件成功、追加 51 件。既存 37 ゴールデンを維持。数値変更の差分マスクはずらす前の参照結果と一致し、PDF 文字注釈も補正座標に対応。補正で相違なしでも元 B を含む 4 枚の証拠画像を保存。macOS Chrome の 1280/390px、補正前後の切り替えとキーボード、外部通信なしを確認。A4・300dpi・600 文字の比較までの中央値は全体移動のみ 344.6ms、数値変更併存 550.9ms。Windows exe 156,463,173 bytes、既存依存とネイティブ DLL のハッシュ・FFmpeg 不在・ZIP 整合を確認。詳細は [T2-4 確認結果](verification/t2-4-alignment.md)。次の T2-4a はタスク化・文書化までで、実装は未着手。

- [ ] T2-4a 承認済みパラメータの外部設定整備（[計画・受け入れ条件](planning/t2-4a-configuration.md)）。コメント付き YAML を継続し、全パラメータの台帳、未対応項目の設定化、説明付き設定例、範囲・優先順位・互換性・実効設定の出力を整備する。T2-4 後の別タスク
- [ ] T2-5 フォント非埋め込みの警告
