...
T0-3 の GitHub Actions では Windows x64 の疎通テスト 2 件が成功している。T2-3 までを含む版は macOS 上で publish・バンドル・ZIP の静的検証まで完了し、Windows では未実行。以下はユーザーの実機での個別条件として、CI と分けて確認状況を管理する。

- [ ] T2-3 の配布 ZIP を展開し、.NET を入れていない PC で exe が単体で動く
- [ ] 単一 exe から pdfium / OpenCvSharpExtern / libSkiaSharp が展開・ロードされる（T2-3 もバンドル内の x64 と SHA-256 の静的確認まで）
...
確認済み（2026-09-21）：ビルド警告 0・全 538 件成功。既存 37 ゴールデンを維持。自作 Type3 フォントを埋め込んだ合成 PDF で日本語・絵文字・座標・失敗時の継続を検証。追加の実 CLI 結合は 8 件。macOS Chrome の 1280/390px、長文とキーボード、外部通信なしを確認。合成 A4 600 単語、1/120/500 クラスタへの片側の注釈は、オープン込み中央値約 19〜21ms。Windows exe 156,440,645 bytes、PdfPig の管理 DLL 7 件と必須ネイティブ DLL を確認し、通知・ZIP の整合も検証。詳細は [T2-3 確認結果](verification/t2-3-text.md)。T2-4 以降は未着手。
