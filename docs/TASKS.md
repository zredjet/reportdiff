...
確認済み（2026-09-21）：拡大縮小せず左上を合わせて右・下を白埋めし、元のサイズ・`SizeMismatch`・警告を保持。ページ順の対応と片側だけのページ、`--pages` の範囲・重複・範囲外エラーを実装。画像・PDF の読み込みから正規化・比較までを含む 55 件を追加し、ビルド警告 0・全 325 件成功。詳細は [T1-7 確認結果](verification/t1-7-pages.md)。T1-8 以降の結果出力・CLI 接続は未着手。
...
- [ ] T1-1〜T1-7 の追加テストを Windows でも確認する（macOS では 325 件成功）
- [ ] 日本語・空白を含むパス、長いパスで動く（T1-5 の画像・T1-6 の PDF の日本語・空白パスは macOS で確認済み）
