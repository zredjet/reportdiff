...
最良ずれの優先順・同点処理・画像境界・検出結果を維持する。PERF-2a はインク再利用と特徴量フィルター範囲縮小、PERF-2b は位置ずれ探索の区間走査と独立グループの並列探索。帯・初期差分の並列化は別段階、SIMD は並列化後の追加効果を測定して採否を決める。大グループ内の候補を無条件に並列評価する方式は、早期終了の利点を失うため採用しない。
