...
T0-3 の GitHub Actions では Windows x64 の疎通テスト 2 件が成功している。T2-4a までを含む版は macOS 上で publish・バンドル・ZIP の静的検証まで完了し、Windows では未実行。以下はユーザーの実機での個別条件として、CI と分けて確認状況を管理する。
...
- [ ] T2-4a 承認済みパラメータの外部設定整備（[計画・受け入れ条件](planning/t2-4a-configuration.md)）。コメント付き YAML を継続し、全パラメータの台帳、未対応項目の設定化、説明付き設定例、範囲・優先順位・互換性・実効設定の出力を整備する。2026-09-21 着手。[台帳と新規 14 項目の具体案](planning/t2-4a-parameter-catalog.md)を作成し、範囲・固定構造との境界もユーザーが承認済み。実装・検証中
- [ ] T2-5 フォント非埋め込みの警告
