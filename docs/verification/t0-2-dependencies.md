...
現在は設定・比較コア（[T1-4 確認結果](t1-4-core.md)）と画像読み込み（[T1-5 確認結果](t1-5-images.md)）まで実装・検証済み。PDF 読み込み・ページ対応・CLI の比較コマンドは未実装。T0-3 で確認した GitHub Actions の Windows x64 / macOS Apple Silicon の結果は、当時の依存疎通テスト 2 件が対象（[T0-3 確認結果](t0-3-ci.md)）。
