...
7. PDFium はスレッドセーフではない。ラスタライズは逐次。比較コアはページ単位、特徴量の帯単位、独立したグループ単位で並列化できる。入力は読み取り専用、出力領域は分離し、例外時も全 worker の終了後に Mat を解放する。帯の生成とグループ探索は段階順に実行し、ページと帯・グループの入れ子で並列数を増やさない
