...
7. PDFium はスレッドセーフではない。ラスタライズは逐次。比較コアはページ単位、特徴量の帯単位、初期候補の行区間単位、独立したグループ単位で並列化できる。入力は読み取り専用、出力領域は分離し、例外時も全 worker の終了後に Mat を解放する。特徴量生成・初期候補抽出・グループ探索は段階順に実行し、ページと帯・行区間・グループの入れ子で並列数を増やさない
   - 100万画素以上の大グループは、ずれ候補を従来順に評価し、1候補の区間ブロックだけを最大4workerで分担する。大グループの全処理後に残りの独立グループを分担し、入れ子にしない。区間索引を共有し、完了印とブロック境界の配列要素は合計1MiB以内かつ索引予算の残りまで。分担できない場合は従来経路へ戻す（SPEC 5.3）
   - 比較後のページA/BのPNG保存は最大2並列、100万画素未満は逐次。補正前B・重ね描き・切り出し・片側ページは逐次とし、他段階と同時実行しない。保存失敗時も両方の終了を待ってからMat・出力ワークスペースを解放する（SPEC 8.1）
