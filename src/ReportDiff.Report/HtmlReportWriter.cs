...
public static partial class HtmlReportWriter
{
    ...
    private static void AppendSettings(StringBuilder html, ReportConfiguration config)
    {
        ...
        html.Append(Metric("インクの局所背景半径", $"{N(config.Ink.BackgroundRadiusMm)} mm"));
        html.Append(Metric("インクの明度差のしきい値", N(config.Ink.ContrastThreshold)));
        ...
        html.Append(Metric("クラスタの読み順の帯幅", $"{N(config.Cluster.ReadingBandMm)} mm"));
        ...
        html.Append(Metric("移動テンプレートの余白", $"{N(config.Move.TemplateMarginMm)} mm"));
        ...
        html.Append(Metric("全体補正の粗い格子の長辺上限", $"{config.Align.CoarseMaxSideSamples} 要素"));
        html.Append(Metric("全体補正の復元段階の探索半径", $"{config.Align.RefineRadiusSamples} 要素"));
        html.Append(Metric("全体補正の支持区画数の下限", config.Align.MinSupportCells));
        html.Append(Metric("全体補正の支持行数の下限", config.Align.MinSupportRows));
        html.Append(Metric("全体補正の支持列数の下限", config.Align.MinSupportColumns));
        html.Append(Metric("全体補正の区画内の黒換算面積", $"{N(config.Align.MinInkAreaMm2)} mm²"));
        html.Append(Metric("PDF 注釈の文字要素数上限（ページ・片側）", config.Text.MaxLettersPerPage));
        html.Append(Metric("PDF 注釈の単語数上限（ページ・片側）", config.Text.MaxWordsPerPage));
